...
class CpbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('cpb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `cpb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM cpb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO cpb (date, title, link, content)"
